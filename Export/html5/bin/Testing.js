...
var Board = function() {
	...
	this.p1_turn = true;
	var mc = flash.Lib.get_current();
	this.sqSize = mc.get_stage().get_width() | 0;
	if(mc.get_stage().get_height() < mc.get_stage().get_width()) this.sqSize = mc.get_stage().get_height() | 0;
	this.map = new Array();
	this.highlited = new Array();
	this.highlitedWinning = new Array();
	var _g = 0;
	while(_g < 81) {
		var x = _g++;
		this.map.splice(x,0,0);
	}
	var _g = 0;
	while(_g < 81) {
		var x = _g++;
		this.highlited.splice(x,0,true);
	}
	var _g = 0;
	while(_g < 81) {
		var x = _g++;
		this.highlitedWinning.splice(x,0,0);
	}
	this.get_stage().addEventListener(flash.events.MouseEvent.CLICK,$bind(this,this.myClick));
};
...
Board.prototype = $extend(flash.display.Sprite.prototype,{
	draw: function() {
		var tileSize = this.sqSize / 9 | 0;
		var mc = flash.Lib.get_current();
		this.drawHighlited(tileSize,mc);
		this.drawLines(tileSize,mc);
		var _g = 0;
		while(_g < 81) {
			var x = _g++;
			if(this.map[x] == 1) this.drawX((x / 9 | 0) * tileSize,(x % 9 | 0) * tileSize,tileSize,mc); else if(this.map[x] == 2) this.drawO((x / 9 | 0) * tileSize,(x % 9 | 0) * tileSize,tileSize,mc);
		}
	}
	,chooseCorner: function(x) {
		var _g = (x / 9 % 3 | 0) * 3 + x % 9 % 3;
		switch(_g) {
		case 0:
			this.highlit(0,0);
			break;
	...
	,highlit: function(x,y) {
		var space = false;
		var _g = 0;
		while(_g < 81) {
			var q = _g++;
			this.highlited[q] = false;
		}
		var _g = 0;
		while(_g < 3) {
			var y1 = _g++;
			...
			while(_g1 < 3) {
				var x1 = _g1++;
				var index = (y1 + y) * 9 + (x1 + x);
				if(this.map[index] == 0 && this.highlitedWinning[index] == 0) {
					space = true;
					this.highlited[index] = true;
				}
			}
		}
		if(!space) {
			...
			while(_g < 81) {
				var q = _g++;
				if(this.map[q] == 0 && this.highlitedWinning[q] == 0) this.highlited[q] = true;
			}
		}
	}
	,highlitWinning: function() {
		var _g = 0;
		while(_g < 3) {
			var y1 = _g++;
			var _g1 = 0;
			while(_g1 < 3) {
				...
				while(_g2 < 3) {
					var rowX = _g2++;
					if(this.highlitedWinning[this.getIndex(x1 * 3,y1 * 3)] == 0) {
						var winner = this.checkIfSomeoneWins(x1 * 3,y1 * 3);
						if(winner != 0) {
							var _g3 = 0;
							while(_g3 < 3) {
								var y2 = _g3++;
								...
								while(_g4 < 3) {
									var x2 = _g4++;
									this.highlitedWinning[this.getIndex(x1 * 3 + x2,y1 * 3 + y2)] = winner;
								}
							}
						}
					}
	...
	,checkIfSomeoneWins: function(x,y) {
		if(this.map[this.getIndex(x,y)] == this.map[this.getIndex(x,y + 1)] && this.map[this.getIndex(x,y + 2)] == this.map[this.getIndex(x,y)]) {
			if(this.map[this.getIndex(x,y)] != 0) return this.map[this.getIndex(x,y)];
		}
		if(this.map[this.getIndex(x + 1,y)] == this.map[this.getIndex(x + 1,y + 1)] && this.map[this.getIndex(x + 1,y + 2)] == this.map[this.getIndex(x + 1,y)]) {
			if(this.map[this.getIndex(x + 1,y)] != 0) return this.map[this.getIndex(x + 1,y)];
		}
		if(this.map[this.getIndex(x + 2,y)] == this.map[this.getIndex(x + 2,y + 1)] && this.map[this.getIndex(x + 2,y + 2)] == this.map[this.getIndex(x + 2,y)]) {
			if(this.map[this.getIndex(x + 2,y)] != 0) return this.map[this.getIndex(x + 2,y)];
		}
		if(this.map[this.getIndex(x,y)] == this.map[this.getIndex(x + 1,y)] && this.map[this.getIndex(x + 2,y)] == this.map[this.getIndex(x,y)]) {
			if(this.map[this.getIndex(x,y)] != 0) return this.map[this.getIndex(x,y)];
		}
		if(this.map[this.getIndex(x,y + 1)] == this.map[this.getIndex(x + 1,y + 1)] && this.map[this.getIndex(x + 2,y + 1)] == this.map[this.getIndex(x,y + 1)]) {
			if(this.map[this.getIndex(x,y + 1)] != 0) return this.map[this.getIndex(x,y + 1)];
		}
		if(this.map[this.getIndex(x,y + 2)] == this.map[this.getIndex(x + 1,y + 2)] && this.map[this.getIndex(x + 2,y + 2)] == this.map[this.getIndex(x,y + 2)]) {
			if(this.map[this.getIndex(x,y + 2)] != 0) return this.map[this.getIndex(x,y + 2)];
		}
		if(this.map[this.getIndex(x,y)] == this.map[this.getIndex(x + 1,y + 1)] && this.map[this.getIndex(x + 2,y + 2)] == this.map[this.getIndex(x,y)]) {
			if(this.map[this.getIndex(x,y)] != 0) return this.map[this.getIndex(x,y)];
		}
		if(this.map[this.getIndex(x + 2,y)] == this.map[this.getIndex(x + 1,y + 1)] && this.map[this.getIndex(x,y + 2)] == this.map[this.getIndex(x + 1,y + 1)]) {
			if(this.map[this.getIndex(x + 1,y + 1)] != 0) return this.map[this.getIndex(x + 1,y + 1)];
		}
		return 0;
	}
	...
	,drawHighlited: function(tileSize,mc) {
		mc.get_graphics().clear();
		var _g = 0;
		while(_g < 81) {
			var x = _g++;
			if(this.highlitedWinning[x] != 0) {
				var posX = x % 9 | 0;
				var posY = x / 9 | 0;
				if(this.highlitedWinning[x] == 1) mc.get_graphics().beginFill(16727614); else mc.get_graphics().beginFill(4620980);
				mc.get_graphics().moveTo(posX * tileSize,posY * tileSize);
				mc.get_graphics().lineTo(posX * tileSize,posY * tileSize + tileSize);
				mc.get_graphics().lineTo(posX * tileSize + tileSize,posY * tileSize + tileSize);
				mc.get_graphics().lineTo(posX * tileSize + tileSize,posY * tileSize);
				mc.get_graphics().endFill();
			}
		}
		var _g = 0;
		while(_g < 81) {
			...
			if(this.highlited[x]) {
				var posX = x % 9 | 0;
				var posY = x / 9 | 0;
				mc.get_graphics().beginFill(15188873);
				mc.get_graphics().moveTo(posX * tileSize,posY * tileSize);
				mc.get_graphics().lineTo(posX * tileSize,posY * tileSize + tileSize);
				mc.get_graphics().lineTo(posX * tileSize + tileSize,posY * tileSize + tileSize);
				mc.get_graphics().lineTo(posX * tileSize + tileSize,posY * tileSize);
	...
	,drawLines: function(tileSize,mc) {
		var width = 2;
		var _g = 0;
		while(_g < 10) {
			var x = _g++;
			if(x % 3 == 0) width = 2; else width = 1;
			mc.get_graphics().beginFill(0);
			mc.get_graphics().moveTo(x * tileSize - width,0);
			mc.get_graphics().lineTo(x * tileSize - width,this.sqSize - 4);
			mc.get_graphics().lineTo(x * tileSize + width,this.sqSize - 4);
			mc.get_graphics().lineTo(x * tileSize + width,0);
			mc.get_graphics().endFill();
		}
		var _g = 0;
		while(_g < 10) {
			var y = _g++;
			if(y % 3 == 0) width = 2; else width = 1;
			mc.get_graphics().beginFill(0);
			mc.get_graphics().moveTo(0,y * tileSize - width);
			mc.get_graphics().lineTo(this.sqSize - 4,y * tileSize - width);
			mc.get_graphics().lineTo(this.sqSize - 4,y * tileSize + width);
			mc.get_graphics().lineTo(0,y * tileSize + width);
			mc.get_graphics().endFill();
		}
	}
	,drawX: function(posY,posX,size,mc) {
		var width = 6;
		var move = 10;
		mc.get_graphics().beginFill(12067876);
		mc.get_graphics().moveTo(posX + move,posY + width + move);
		mc.get_graphics().lineTo(posX + width + move,posY + move);
		mc.get_graphics().lineTo(posX + size - move,posY + size - width - move);
		mc.get_graphics().lineTo(posX + size - width - move,posY + size - move);
		mc.get_graphics().lineTo(posX + move,posY + width + move);
		mc.get_graphics().endFill();
		mc.get_graphics().beginFill(12067876);
		mc.get_graphics().moveTo(posX + size - width - move,posY + move);
		mc.get_graphics().lineTo(posX + size - move,posY + width + move);
		mc.get_graphics().lineTo(posX + width + move,posY + size - move);
		mc.get_graphics().lineTo(posX + move,posY + size - width - move);
		mc.get_graphics().moveTo(posX + size - width - move,posY + move);
		mc.get_graphics().endFill();
	}
	,drawO: function(posY,posX,size,mc) {
		mc.get_graphics().beginFill(3091858);
		mc.get_graphics().drawCircle(posX + size / 2,posY + size / 2,size / 2 - 10);
		mc.get_graphics().endFill();
		if(this.highlitedWinning[(posY / size | 0) * 9 + (posX / size | 0)] == 1) mc.get_graphics().beginFill(16727614); else if(this.highlitedWinning[(posY / size | 0) * 9 + (posX / size | 0)] == 2) mc.get_graphics().beginFill(4620980); else mc.get_graphics().beginFill(16777215);
		mc.get_graphics().drawCircle(posX + size / 2,posY + size / 2,size / 2 - 19);
		mc.get_graphics().endFill();
	}
	,myClick: function(eventObject) {
		var clicked = (eventObject.stageY / (this.sqSize / 9) | 0) * 9 + (eventObject.stageX / (this.sqSize / 9) | 0);
		if(this.highlited[clicked]) {
			if(this.p1_turn) this.map[clicked] = 1; else this.map[clicked] = 2;
			this.p1_turn = !this.p1_turn;
			this.highlited[clicked] = false;
			this.highlitWinning();
			this.chooseCorner(clicked);
		}
		this.draw();
	}
	,__class__: Board
